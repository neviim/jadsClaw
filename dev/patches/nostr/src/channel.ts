...
// Check if a sender pubkey is in the allowlist
function isSenderAllowed(senderPubkey: string, allowFrom: string[]): boolean {
  ...
  return allowFrom.some((entry) => {
    try {
      const normalized = normalizePubkey(entry.replace(/^nostr:/i, ""));
      return normalized === normalizedSender;
    } catch {
      return entry.toLowerCase() === normalizedSender;
    }
  });
}
...
export const nostrPlugin: ChannelPlugin<ResolvedNostrAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // Status sink for tracking activity timestamps
      ...
      const bus = await startNostrBus({
        ...
        // FIX: Use the standard OpenClaw inbound pipeline instead of the
        // non-existent runtime.channel.reply.handleInboundMessage() method.
        onMessage: async (senderPubkey, text, reply) => {
          ...
          // --- DM policy / allowlist / pairing gate ---
          const dmPolicy = account.config.dmPolicy ?? "pairing";
          const configAllowFrom = (account.config.allowFrom ?? []).map((v: unknown) => String(v));

          const shouldComputeAuth = core.channel.commands.shouldComputeCommandAuthorized(text, cfg);
          const storeAllowFrom =
            dmPolicy !== "open" || shouldComputeAuth
              ? await core.channel.pairing.readAllowFromStore("nostr").catch(() => [])
              : [];
          const effectiveAllowFrom = [...configAllowFrom, ...storeAllowFrom];
          const senderAllowed = isSenderAllowed(senderPubkey, effectiveAllowFrom);
          ...
          if (dmPolicy !== "open" && !senderAllowed) {
            if (dmPolicy === "pairing") {
              ...
              if (created) {
                ctx.log?.info(`[${account.accountId}] Nostr pairing request sender=${senderPubkey}`);
                try {
                  await reply(
              ...
              logInboundDrop({
                log: (msg: string) => ctx.log?.info(msg),
                channel: "nostr",
                reason: `DM not allowed (dmPolicy=${dmPolicy})`,
                target: senderPubkey,
              });
            }
            return;
          ...
          await core.channel.session.recordInboundSession({
            ...
            onRecordError: (err: unknown) => {
              ctx.log?.error(`[${account.accountId}] Nostr session record failed: ${String(err)}`);
            },
          });
          ...
          await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
              ...
              deliver: async (payload: { text?: string }) => {
                ...
                const message = core.channel.text.convertMarkdownTables(replyText, tableMode);
                await reply(message);
                statusSink({ lastOutboundAt: Date.now() });
              },
