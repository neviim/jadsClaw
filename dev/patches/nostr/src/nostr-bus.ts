...
export function validatePrivateKey(key: string): Uint8Array {
  ...
  if (trimmed.startsWith("nsec1")) {
    ...
    // nostr-tools CJS may return hex string instead of Uint8Array
    if (typeof decoded.data === "string") {
      const bytes = new Uint8Array(32);
      for (let i = 0; i < 32; i++) {
        bytes[i] = parseInt(decoded.data.slice(i * 2, i * 2 + 2), 16);
      }
      return bytes;
    }
    return decoded.data;
...
export async function startNostrBus(options: NostrBusOptions): Promise<NostrBusHandle> {
  ...
  // FIX: Pass filter as single object instead of array to work around
  // nostr-tools@2.23.0 double-wrapping bug in subscribeMany/subscribe chain.
  // The bug: subscribeMany -> subscribe -> subscribeMap wraps the filter array
  // inside another array, producing [[filter]] instead of [filter], which causes
  // relays to reject with "provided filter is not an object".
